...
public:
  ...
  // Local additions for Point-LIO map statistics diagnostics.
  std::size_t numVoxels() const { return grids_.size(); }

  std::size_t numRepresentatives() const {
    std::size_t total = 0;
    for (const auto& kv : data_) {
      const OctVoxType& voxel = kv.second;
      for (uint8_t i = 0; i < 8; ++i) {
        if (voxel.counts_[i] != OctVoxType::UNINIT_MASK) {
          ++total;
        }
      }
    }
...
// The upstream saveMap() depends on the Super-LIO ROS package ROOT macro and
// PCL I/O; Point-LIO uses its own map persistence, so this adapter disables
// the unused implementation (see third_party/super_lio_octvox/UPSTREAM.md).
template<typename Point, typename Scalar>
