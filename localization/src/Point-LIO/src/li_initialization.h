#pragma once

#include <common_lib.h>
#include "Estimator.h"
#include <atomic>
#define MAXN                (720000)

extern bool data_accum_finished, data_accum_start, online_calib_finish, refine_print;
extern int frame_num_init;
extern double time_lag_IMU_wtr_lidar, move_start_time, online_calib_starts_time; //, mean_acc_norm = 9.81;

extern double timediff_imu_wrt_lidar;
extern bool timediff_set_flg;
extern V3D gravity_lio;
extern mutex mtx_buffer;
extern condition_variable sig_buffer;
extern int scan_count;
extern int frame_ct, wait_num;
extern std::deque<PointCloudXYZI::Ptr>  lidar_buffer;
extern std::deque<double>               time_buffer;
extern std::deque<sensor_msgs::Imu::Ptr> imu_deque;
extern std::mutex m_time;
extern bool lidar_pushed, imu_pushed;
extern double imu_first_time;
extern bool lose_lid;
extern sensor_msgs::Imu imu_last, imu_next;
extern PointCloudXYZI::Ptr  ptr_con;
extern double T1[MAXN], s_plot[MAXN], s_plot2[MAXN], s_plot3[MAXN], s_plot11[MAXN];
extern std::atomic<unsigned long> dropped_lidar_scans_total;

// Estimator-independent recovery cloud (fixed LiDAR->body extrinsic only).
extern ros::Publisher recovery_cloud_pub;
extern double last_recovery_cloud_publish_time;

// Runtime diagnostics snapshot. Every field is read under mtx_buffer in a
// single critical section so the published metric is internally consistent.
struct SensorBufferStats
{
    double latest_lidar_stamp = -1.0;
    double latest_imu_stamp = -1.0;
    double lidar_front_stamp = -1.0;
    double lidar_back_stamp = -1.0;
    size_t lidar_queue_size = 0;
    size_t imu_queue_size = 0;
};

SensorBufferStats getSensorBufferStats();

// Thread-safe accessors for the shared IMU deque. laserMapping.cpp must use
// these instead of touching imu_deque directly while AsyncSpinner is active.
bool imuBufferEmpty();
bool imuBufferFront(sensor_msgs::Imu &out);
bool imuBufferPopFront();
bool popImuFrontAndGetNext(sensor_msgs::Imu &next);
size_t imuBufferSize();
size_t dropStaleLidarFramesForCatchup();

// extern sensor_msgs::Imu::ConstPtr imu_last_ptr;

void standard_pcl_cbk(const sensor_msgs::PointCloud2::ConstPtr &msg); 
void livox_pcl_cbk(const livox_ros_driver::CustomMsg::ConstPtr &msg); 
void imu_cbk(const sensor_msgs::Imu::ConstPtr &msg_in); 
bool sync_packages(MeasureGroup &meas);

// #endif
