...
std::string local_map_backend_name = "ivox";
float octvox_resolution = 0.4f;
int octvox_capacity = 50000;

std::vector<double> extrinT(3, 0.0);
std::vector<double> extrinR(9, 0.0);
state_input state_in;
state_output state_out;
std::string lid_topic, imu_topic, publish_world_frame = "camera_init";
...
double odometry_max_publish_hz = 50.0;
std::string odometry_tf_child_frame = "body";
double odometry_tf_child_to_body_x_m = 0.0;
double odometry_tf_child_to_body_y_m = 0.0;
double odometry_tf_child_to_body_z_m = 0.0;
...
bool   scan_world_publish_en = true, scan_world_downsample_publish_en = true;
bool   scan_bodyframe_pub_en = false;
bool   recovery_cloud_enable = true;
double recovery_cloud_max_hz = 5.0;
bool   enable_lidar_backlog_catchup = false;
bool   enable_map_backend_timing = false;
double lidar_backlog_trigger_s = 0.20;
double lidar_backlog_target_s = 0.10;
size_t min_lidar_frames_to_keep = 1;
int    sensor_callback_threads = 2;
int    lidar_sub_queue_size = 20;
int    imu_sub_queue_size = 1000;
int    odom_pub_queue_size = 10;
int    cloud_pub_queue_size = 2;
int    path_pub_queue_size = 1;
...
void readParameters(ros::NodeHandle &nh)
{
  ...
  nh.param<std::string>("publish/world_frame", publish_world_frame,
  ...
  nh.param<double>("odometry/max_publish_hz", odometry_max_publish_hz, 50.0);
  nh.param<std::string>("odometry/tf_child_frame", odometry_tf_child_frame, std::string("body"));
  nh.param<double>("odometry/tf_child_to_body_x_m", odometry_tf_child_to_body_x_m, 0.0);
  nh.param<double>("odometry/tf_child_to_body_y_m", odometry_tf_child_to_body_y_m, 0.0);
  nh.param<double>("odometry/tf_child_to_body_z_m", odometry_tf_child_to_body_z_m, 0.0);
  ...
  nh.param<bool>("publish/scan_world_publish_en", scan_world_publish_en,
                 scan_pub_en);
  nh.param<bool>("publish/scan_world_downsample_publish_en",
                 scan_world_downsample_publish_en, scan_pub_en);
  nh.param<bool>("publish/scan_bodyframe_pub_en", scan_bodyframe_pub_en,
                 scan_body_pub_en);
  nh.param<bool>("recovery_cloud/enable", recovery_cloud_enable, true);
  nh.param<double>("recovery_cloud/max_publish_hz", recovery_cloud_max_hz,
                   5.0);
  nh.param<bool>("runtime_pos_log_enable", runtime_pos_log, 0);
  nh.param<bool>("pcd_save/pcd_save_en", pcd_save_en, false);
  nh.param<int>("pcd_save/interval", pcd_save_interval, -1);
  nh.param<bool>("realtime/enable_lidar_backlog_catchup",
                 enable_lidar_backlog_catchup, false);
  nh.param<bool>("realtime/enable_map_backend_timing",
                 enable_map_backend_timing, false);
  nh.param<double>("realtime/lidar_backlog_trigger_s",
                   lidar_backlog_trigger_s, 0.20);
  nh.param<double>("realtime/lidar_backlog_target_s",
                   lidar_backlog_target_s, 0.10);
  int keep_frames = 1;
  nh.param<int>("realtime/min_lidar_frames_to_keep", keep_frames, 1);
  min_lidar_frames_to_keep =
      static_cast<size_t>(keep_frames > 1 ? keep_frames : 1);
  nh.param<int>("realtime/sensor_callback_threads", sensor_callback_threads,
                2);
  nh.param<int>("realtime/lidar_sub_queue_size", lidar_sub_queue_size, 20);
  nh.param<int>("realtime/imu_sub_queue_size", imu_sub_queue_size, 1000);
  nh.param<int>("realtime/odom_pub_queue_size", odom_pub_queue_size, 10);
  nh.param<int>("realtime/cloud_pub_queue_size", cloud_pub_queue_size, 2);
  nh.param<int>("realtime/path_pub_queue_size", path_pub_queue_size, 1);
  sensor_callback_threads = std::max(sensor_callback_threads, 1);
  lidar_sub_queue_size = std::max(lidar_sub_queue_size, 1);
  imu_sub_queue_size = std::max(imu_sub_queue_size, 1);
  odom_pub_queue_size = std::max(odom_pub_queue_size, 1);
  cloud_pub_queue_size = std::max(cloud_pub_queue_size, 1);
  path_pub_queue_size = std::max(path_pub_queue_size, 1);
  ...
  nh.param<std::string>("mapping/local_map_backend", local_map_backend_name,
                        "ivox");
  nh.param<float>("mapping/octvox_resolution", octvox_resolution, 0.4f);
  nh.param<int>("mapping/octvox_capacity", octvox_capacity, 50000);
