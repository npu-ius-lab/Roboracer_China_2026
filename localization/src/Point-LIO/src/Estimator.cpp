...
std::shared_ptr<LocalMapBackend> local_map_backend_ = nullptr;
MapBackendStats map_backend_stats;
...
void h_model_input(state_input &s, Eigen::Matrix3d cov_p, Eigen::Matrix3d cov_R, esekfom::dyn_share_modified<double> &ekfom_data)
{
	...
	for (int j = 0; j < time_seq[k]; j++)
	{
		...
		p_world << point_world_j.x, point_world_j.y, point_world_j.z;
		{
            ...
            {
                bool ok = false;
                if (enable_map_backend_timing)
                {
                    const auto knn_begin = std::chrono::steady_clock::now();
                    ok = local_map_backend_->getClosestPoint(
                        point_world_j, points_near, NUM_MATCH_POINTS, 5.0);
                    const double knn_ms =
                        std::chrono::duration<double, std::milli>(
                            std::chrono::steady_clock::now() - knn_begin)
                            .count();
                    map_backend_stats.knn_total_ms += knn_ms;
                    map_backend_stats.knn_max_ms =
                        std::max(map_backend_stats.knn_max_ms, knn_ms);
                }
                else
                {
                    ok = local_map_backend_->getClosestPoint(
                ...
                map_backend_stats.knn_queries += 1;
                map_backend_stats.knn_success += ok ? 1 : 0;
            }
