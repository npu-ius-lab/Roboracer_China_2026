...
std::string pcd_file;
bool save_filtered_clouds = false;
...
int main(int argc, char **argv) {
  ...
  private_nh.param("file_directory", file_directory, std::string(""));

  private_nh.param("file_name", file_name, std::string("map"));

  private_nh.param("pcd_file", pcd_file, std::string(""));
  if (pcd_file.empty()) {
  ...
  private_nh.param("save_filtered_clouds", save_filtered_clouds, false);

  if (thre_z_max <= thre_z_min || map_resolution <= 0.0 ||
      thre_radius <= 0.0 || thres_point_count < 0) {
    ROS_ERROR("Invalid filter or map parameters");
    return 1;
  }
...
void RadiusOutlierFilter(const pcl::PointCloud<pcl::PointXYZ>::Ptr &pcd_cloud0,
  ...
  if (save_filtered_clouds) {
    pcl::io::savePCDFile<pcl::PointXYZ>(
        file_directory + "map_radius_filter.pcd", *cloud_after_Radius);
  }
...
void SetMapTopicMsg(const pcl::PointCloud<pcl::PointXYZ>::Ptr cloud,
  ...
  //设置栅格地图大小
  msg.info.width =
      static_cast<uint32_t>(std::ceil((x_max - x_min) / map_resolution)) + 1;
  msg.info.height =
      static_cast<uint32_t>(std::ceil((y_max - y_min) / map_resolution)) + 1;
  ...
  for (int iter = 0; iter < cloud->points.size(); iter++) {
    ...
    if (j < 0 || j >= static_cast<int>(msg.info.height))
      continue;
