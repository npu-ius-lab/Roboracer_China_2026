#!/usr/bin/env python3
"""V5 Chaoche overtaking planner with stable authorization gating."""
...
from f1tenth_overtake import OvertakePlanner, PeriodicTrack, PlannerConfig, VehicleState
...
class OvertakeShadowNode:
    def __init__(self) -> None:
        ...
        self.minimum_dynamic_overtake_speed = float(
            rospy.get_param("~authorization/minimum_dynamic_ego_speed_mps", 1.00)
        )
        self.minimum_static_overtake_speed = float(
            rospy.get_param("~authorization/minimum_static_ego_speed_mps", 0.60)
        )
        self.minimum_authorization_closing_speed = float(
            rospy.get_param("~authorization/minimum_closing_speed_mps", 0.10)
        )
        self.candidate_stability = float(
            rospy.get_param("~authorization/candidate_stability_s", 0.50)
        )
        self.static_id_base = int(
            rospy.get_param("~authorization/static_target_id_base", 1000000000)
        )
        ...
        self.authorization_since = None

        self.left_pub = rospy.Publisher(
            rospy.get_param("~topics/left_path", "/overtake/candidate_left"), Path, queue_size=1
        )
        self.right_pub = rospy.Publisher(
            rospy.get_param("~topics/right_path", "/overtake/candidate_right"), Path, queue_size=1
        )
        self.selected_pub = rospy.Publisher(
            rospy.get_param("~topics/selected_path", "/overtake/selected_path"), Path, queue_size=1
        )
        self.marker_pub = rospy.Publisher(
            rospy.get_param("~topics/markers", "/overtake/markers"), MarkerArray, queue_size=1
        )
        self.diagnostic_pub = rospy.Publisher(
            rospy.get_param("~topics/diagnostics", "/overtake/diagnostics"), String, queue_size=1
        )
        self.valid_area_pub = rospy.Publisher(
            rospy.get_param("~topics/valid_area", "/overtake/valid_area"),
            Bool,
            queue_size=1,
            latch=True,
        )
        ...
        rospy.logwarn(
            "Overtake shadow node ready: track=%.2fm. It publishes paths/diagnostics only and has no command output.",
            self.track.length,
        )
    ...
    def publish_result(
        ...
        **timing,
        ...
        selected_side, authorization_reason, static_obstacle = (
            self.authorized_side(result, target_id, stamp)
        )
        selected_candidate = None
        if selected_side == "left":
            selected_candidate = result.left
        elif selected_side == "right":
            selected_candidate = result.right
        overtake_authorized = selected_candidate is not None
        self.left_pub.publish(self.path_message(result.left, stamp))
        self.right_pub.publish(self.path_message(result.right, stamp))
        self.selected_pub.publish(self.path_message(selected_candidate, stamp))
        markers = MarkerArray()
        clear = Marker()
        clear.action = Marker.DELETEALL
        markers.markers.append(clear)
        markers.markers.append(self.candidate_marker(result.left, 1, stamp, selected_side == "left"))
        markers.markers.append(self.candidate_marker(result.right, 2, stamp, selected_side == "right"))
    ...
    def authorized_side(self, result, target_id: int, stamp: rospy.Time):
        static_obstacle = target_id >= self.static_id_base
        selected_side = result.selected_side if result.selected is not None else "none"
        minimum_speed = (
            self.minimum_static_overtake_speed
            if static_obstacle
            else self.minimum_dynamic_overtake_speed
        )
        closing_ready = bool(
            static_obstacle
            or result.closing_speed >= self.minimum_authorization_closing_speed
        )
        base_ready = bool(
            selected_side in ("left", "right")
            and result.ego_vs >= minimum_speed
            and closing_ready
        )
        if not base_ready:
            with self.lock:
            ...
            if selected_side in ("left", "right"):
                if result.ego_vs < minimum_speed:
                    reason = "follow_until_minimum_overtake_speed"
                else:
                    reason = "follow_target_not_slower"
            else:
                reason = result.reason
            return "none", reason, static_obstacle

        key = (int(target_id), selected_side)
        now = stamp.to_sec()
        with self.lock:
            if key != self.authorization_key or self.authorization_since is None:
                self.authorization_key = key
                self.authorization_since = now
            stable_for = max(0.0, now - self.authorization_since)
        if stable_for < self.candidate_stability:
            return "none", "candidate_stabilizing", static_obstacle
        return selected_side, result.reason, static_obstacle
...
def main() -> None:
    rospy.init_node("overtake_shadow")
    OvertakeShadowNode()
    rospy.spin()
