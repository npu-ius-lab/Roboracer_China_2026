...
if [[ $# -lt 2 ]]; then
  echo "usage: $0 BAG_PATH OUTPUT_JSON [MASTER_PORT=11323] [PLAY_RATE=1.0]" >&2
  exit 2
fi
...
ROS_LOG_DIR="${AVOIDANCE_TEST_LOG_DIR:-/tmp/avoidance_aligned_cloud_${MASTER_PORT}}"

for path in "$BAG_PATH" "$RACELINE" "$PERCEPTION_WS/devel/setup.bash"; do
  if [[ ! -f "$path" ]]; then
...
sleep 2
echo "[aligned-cloud-test] bag=$BAG_PATH rate=$PLAY_RATE master=$ROS_MASTER_URI"
rosbag play "$BAG_PATH" --quiet --clock --rate "$PLAY_RATE" \
  --topics /localization/aligned_scan /localization/odom /tf /tf_static
sleep 1
kill -TERM "$OBSERVER_PID" 2>/dev/null || true
wait "$OBSERVER_PID" 2>/dev/null || true
echo "[aligned-cloud-test] result=$OUTPUT_JSON logs=$ROS_LOG_DIR"
