...
LAUNCH="$ROOT/src/f1tenth_dynamic_mpcc/launch/hardware_mpcc_stable_roboracer_safe_start_fast2_candidate.launch"
...
"$STABLE" --check-only
check_hash "83947565a49f41a23cfb5dccda2b143f71feb26c134a594d0b8a02b214a9d022" "$LAUNCH"
check_hash "5f2ae19f7f8780ee7bb8969264a2cd615f809d5a8d32a214f5c58b86e7b9012f" "$SUPERVISOR"
...
if [[ "$ACTION" == "--check-only" ]]; then
  echo "[OK] Isolated RoboRacer safe-start fast2 candidate hashes match"
  echo "[OK] race-start switch=$RACE_START_ENABLED; release=0.50 m/s; PP<=2.00 m/s"
  echo "[OK] gentle profile applies to race_start only; recovery=frozen RoboRacer"
...
if [[ "$ACTION" == "--prepare-only" ]]; then
  echo "[OK] RoboRacer safe-start fast2 candidate solver is ready"
  exit 0
fi
...
if [[ "${MPCC_RECORD_BAG:-false}" == "true" ]]; then
  mkdir -p "$ROOT/bags"
  BAG_PATH="${MPCC_RECORD_BAG_PATH:-$ROOT/bags/stable_roboracer_safe_start_fast2_${MODE_TAG}_${STAMP}.bag}"
  "$RECORDER" "$BAG_PATH" &
  RECORD_PID=$!
  echo "[RECORD] RoboRacer safe-start fast2 bag -> $BAG_PATH"
fi

echo "============================================================"
echo "[ROBORACER_SAFE_START_FAST2] ISOLATED CANDIDATE; frozen RoboRacer is untouched"
echo "[ROBORACER_SAFE_START_FAST2] race-start enabled=$RACE_START_ENABLED"
echo "[ROBORACER_SAFE_START_FAST2] E-stop release probe=0.50 m/s"
echo "[ROBORACER_SAFE_START_FAST2] PP max=2.00 m/s; PP accel/decel=1.50/3.00 m/s^2"
echo "[ROBORACER_SAFE_START_FAST2] scope=race_start ONLY; carry recovery=frozen RoboRacer"
echo "[ROBORACER_SAFE_START_FAST2] corridor slow/abort margin=0.15/0.02 m"
echo "[ROBORACER_SAFE_START_FAST2] extracted-track safety gate is ENABLED"
echo "[ROBORACER_SAFE_START_FAST2] runtime MPCC cap=$RESIDUAL_MPCC_SPEED_CAP m/s"
if [[ "$RACE_START_ENABLED" == "false" ]]; then
  echo "[ROBORACER_SAFE_START_FAST2] grid launch mode OFF; placement falls back to guarded recovery"
fi
echo "[ROBORACER_SAFE_START_FAST2] state=/automatic_relaunch_supervisor/state"
echo "[WARNING] Hold the physical E-stop until WAIT_RELEASE is shown."
echo "============================================================"

roslaunch f1tenth_dynamic_mpcc \
  hardware_mpcc_stable_roboracer_safe_start_fast2_candidate.launch \
  ...
  telemetry_path:="$ROOT/log/hardware_stable_roboracer_safe_start_fast2_${MODE_TAG}_${STAMP}.jsonl" \
  lap_timer:="${AUTO_RELAUNCH_LAP_TIMER:-true}" \
  lap_log_path:="$ROOT/log/laps_stable_roboracer_safe_start_fast2_${MODE_TAG}_${STAMP}.csv"
