...
if [[ "$ACTION" == "--check-only" ]]; then
  echo "[OK] V3pro inputs match"
  echo "[OK] Stable V3 normal driving + Stable V5 carry/relaunch behavior"
  exit 0
fi
...
GENERATED_DIR="${MPCC_GENERATED_DIR:-$HOME/.cache/f1tenth_residual_mpcc/acados_stable_v3pro}"
...
if [[ "${MPCC_RECORD_BAG:-false}" == "true" ]]; then
  mkdir -p "$ROOT/bags"
  BAG_PATH="${MPCC_RECORD_BAG_PATH:-$ROOT/bags/stable_v3pro_${STAMP}.bag}"
  "$RECORDER" "$BAG_PATH" &
  RECORD_PID=$!
...
echo "[V3PRO] Stable V3 raceline/residual/speed/braking/safety preserved"
echo "[V3PRO] MPCC solve=40 Hz; command publisher/supervisor=50 Hz"
echo "[V3PRO] contour/heading/rate-cost=45.0/1.0/0.60; steering rate=3.4 rad/s"
echo "[V3PRO] profile decel=3.0; publisher accel/decel=1.5/2.0 m/s^2"
echo "[V3PRO] runtime cap=${RESIDUAL_MPCC_SPEED_CAP} m/s; track=racelinev3"
...
echo "[V3PRO] Stable V3 and Stable V5 remain unchanged"
...
LAUNCH_ARGS=(
  ...
  telemetry_path:="$ROOT/log/hardware_stable_v3pro_${STAMP}.jsonl"
  lap_timer:="${AUTO_RELAUNCH_LAP_TIMER:-true}"
  lap_log_path:="$ROOT/log/laps_stable_v3pro_${STAMP}.csv"
)
