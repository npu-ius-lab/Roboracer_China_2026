...
TRACK="racelinev3_stable_v3pro_control_curvature_speed_candidate"
CONTROLLER_CONFIG="candidates/stable_v3pro/controller.yaml"
...
MANIFEST="$ROOT/src/f1tenth_dynamic_mpcc/data/tracks/$TRACK/MANIFEST.json"
GENERATOR="$ROOT/scripts/generate_stable_v3pro_control_curvature_speed_candidate.py"
...
check_hash "197a5e6db20b2a1dbea0782f9812926de550642e4beaf5647973404b4bbe7fce" "$RACELINE"
check_hash "e000850ddcd65b59a0f155f9d6e3f4805df98c7ac177100a4a5fc30599f014f3" "$SPEED_ZONES"
check_hash "d9cb69182c2b50a27c1d40185f97efc01a644b8f001348c12b7bea12a6b96a41" "$MANIFEST"
check_hash "375a12f1c7c746da7de2733a91d98f0104cc202164233153802a7bfa85488dd7" "$GENERATOR"
...
if [[ "$ACTION" == "--check-only" ]]; then
  echo "[OK] V3pro control-curvature-speed candidate inputs match"
  echo "[OK] V3pro controller/residual/relaunch behavior remains frozen"
  exit 0
fi
...
GENERATED_DIR="${MPCC_GENERATED_DIR:-$HOME/.cache/f1tenth_residual_mpcc/acados_stable_v3pro_control_curvature_speed_candidate}"
...
if [[ "$PREPARE_ONLY" == "true" ]]; then
  echo "[OK] V3pro control-curvature-speed candidate solver is ready; hardware was not started"
  exit 0
fi
...
if [[ "${MPCC_RECORD_BAG:-false}" == "true" ]]; then
  mkdir -p "$ROOT/bags"
  BAG_PATH="${MPCC_RECORD_BAG_PATH:-$ROOT/bags/stable_v3pro_control_curvature_speed_candidate_${STAMP}.bag}"
  "$RECORDER" "$BAG_PATH" &
  RECORD_PID=$!
  echo "[RECORD] V3pro control-curvature-speed candidate bag -> $BAG_PATH"
  trap 'kill -INT "$RECORD_PID" 2>/dev/null || true' EXIT
fi

echo "[V3PRO_RACELINE_CANDIDATE] Frozen V3pro controller/residual/braking/safety preserved"
echo "[V3PRO_RACELINE_CANDIDATE] MPCC solve=40 Hz; command publisher/supervisor=50 Hz"
echo "[V3PRO_RACELINE_CANDIDATE] contour/heading/rate-cost=45.0/1.0/0.60; steering rate=3.4 rad/s"
echo "[V3PRO_RACELINE_CANDIDATE] profile decel=3.0; publisher accel/decel=1.5/2.0 m/s^2"
echo "[V3PRO_RACELINE_CANDIDATE] runtime cap=${RESIDUAL_MPCC_SPEED_CAP} m/s; track=$TRACK"
echo "[V3PRO_RACELINE_CANDIDATE] standard safe windows=3.3/3.5 m/s; tight bends=3.0 m/s"
...
echo "[V3PRO_RACELINE_CANDIDATE] Stable V3, V3pro and Stable V5 remain unchanged"
...
LAUNCH_ARGS=(
  ...
  telemetry_path:="$ROOT/log/hardware_stable_v3pro_control_curvature_speed_candidate_${STAMP}.jsonl"
  lap_timer:="${AUTO_RELAUNCH_LAP_TIMER:-true}"
  lap_log_path:="$ROOT/log/laps_stable_v3pro_control_curvature_speed_candidate_${STAMP}.csv"
)
