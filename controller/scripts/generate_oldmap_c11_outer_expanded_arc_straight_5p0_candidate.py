#!/usr/bin/env python3
"""Generate the additive old-map C11-expanded 5.0 m/s speed candidate."""
...
OUTPUT_DIR = PACKAGE / "data/tracks/raceline_smooth_c11_outer_boundary_expanded_arc_straight_5p0_candidate"
OUTPUT_TRACK = OUTPUT_DIR / "raceline.csv"
OUTPUT_ZONES = OUTPUT_DIR / "speed_zones.csv"
CONTROLLER = PACKAGE / "config/candidates/stable_roboracer_c11_outer_expanded_arc_straight_5p0/controller.yaml"
VEHICLE = PACKAGE / "config/candidates/stable_roboracer_c11_outer_expanded_arc_straight_5p0/vehicle.yaml"
...
def main() -> None:
    controller = yaml.safe_load(CONTROLLER.read_text(encoding="utf-8"))
    if float(controller["speed_planning"]["max_speed_mps"]) != 5.0:
        raise RuntimeError("MPCC speed-planning ceiling must be 5.0 m/s")
    if float(controller["safety"]["debug_speed_max_mps"]) != 5.0:
        raise RuntimeError("active debug-mode safety ceiling must be 5.0 m/s")
    if float(controller["pure_pursuit_candidate"]["max_speed_mps"]) != 4.0:
        raise RuntimeError("PP ceiling must remain at the 4.5 baseline value of 4.0 m/s")
    ...
    for row in rows:
        ...
        if zone in FAST_ZONES:
            row["speed_limit_mps"] = "5.000000000"
    ...
    if max(reproduced.speed_nodes) < 5.0 - 1.0e-6:
        raise RuntimeError("candidate does not reach 5.0 m/s")
    for row in rows:
