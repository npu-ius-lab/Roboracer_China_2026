...
LAUNCH="$ROOT/src/f1tenth_dynamic_mpcc/launch/hardware_mpcc_stable_roboracer_h2h_safe_start_fast2_candidate.launch"
...
for path in "$BASE_START" "$LAUNCH" "$RUNTIME_CONFIG" "$SUPERVISOR" \
    "$POLICY" "$FOLLOW_GATE" "$RECORDER" "$PERCEPTION_HELPER"; do
...
if [[ "$ACTION" == "--check-only" ]]; then
  echo "[OK] isolated RoboRacer H2H safe-start-fast2 candidate"
...
if [[ "${MPCC_RECORD_BAG:-false}" == "true" ]]; then
  BAG_PATH="${MPCC_RECORD_BAG_PATH:-$ROOT/bags/roboracer_h2h_safe_start_fast2_${STAMP}.bag}"
  "$RECORDER" "$BAG_PATH" &
  pids+=("$!")
...
roslaunch f1tenth_dynamic_mpcc \
  hardware_mpcc_stable_roboracer_h2h_safe_start_fast2_candidate.launch \
