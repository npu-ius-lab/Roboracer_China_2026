...
BASE_LAUNCHER="$ROOT/scripts/start_mpcc_hardware_stable_v3_5mps_arc_straight_std3_candidate.sh"
CANDIDATE="stable_v3_5mps_arc_straight_std3_auto_relaunch_candidate"
...
LAUNCH="$ROOT/src/f1tenth_dynamic_mpcc/launch/hardware_mpcc_stable_v3_5mps_auto_relaunch_candidate.launch"
...
# This proves that the accepted 5/3 m/s launcher and every frozen stableV3
# input it depends on remain byte-for-byte unchanged.
"$BASE_LAUNCHER" --check-only
check_hash "54f25897e95f1d632ee067fed1e21c044f05790f36ba0f6b3d4678ca1d1db805" "$CONTROLLER"
check_hash "abd6874fd0c2e6500b07e9d74981d6005aa9cc018d07624ce4d9b67668aac016" "$LAUNCH"
...
if [[ "$ACTION" == "--check-only" ]]; then
  echo "[OK] original 5/3 m/s candidate is unchanged"
  echo "[OK] isolated automatic carry/relaunch candidate hashes match"
  exit 0
fi
...
GENERATED_DIR="${MPCC_GENERATED_DIR:-$HOME/.cache/f1tenth_residual_mpcc/acados_stable_v3_5mps_auto_relaunch}"
...
if [[ "${MPCC_RECORD_BAG:-false}" == "true" ]]; then
  mkdir -p "$ROOT/bags"
  BAG_PATH="${MPCC_RECORD_BAG_PATH:-$ROOT/bags/stable_v3_5mps_auto_relaunch_${STAMP}.bag}"
  "$RECORDER" "$BAG_PATH" &
  RECORD_PID=$!
...
echo "[AUTO_RELAUNCH] original 5 m/s launcher and stableV3 remain unchanged"
...
LAUNCH_ARGS=(
  ...
  telemetry_path:="$ROOT/log/hardware_stable_v3_5mps_auto_relaunch_${STAMP}.jsonl"
  lap_timer:="${AUTO_RELAUNCH_LAP_TIMER:-true}"
  lap_log_path:="$ROOT/log/laps_stable_v3_5mps_auto_relaunch_${STAMP}.csv"
)

if [[ -n "$RECORD_PID" ]]; then
  roslaunch f1tenth_dynamic_mpcc \
    hardware_mpcc_stable_v3_5mps_auto_relaunch_candidate.launch \
    "${LAUNCH_ARGS[@]}"
else
