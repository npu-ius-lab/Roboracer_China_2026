...
RESIDUAL="$ROOT/src/f1tenth_dynamic_mpcc/config/residual/residual_mpcc_track_h3_candidate.yaml"
...
check_hash() {
  ...
  if [[ "$actual" != "$expected" ]]; then
    echo "[FAIL] racelineV3 H3 candidate file changed: $path" >&2
...
# Pin every input used by this candidate. It is independent of stableV2 and of
# the racelinev1 H3 candidate, so either profile can still be launched later.
check_hash "a78197b6c064c9f5f02bf6ed5d08dae0bbf231958d917a69a0ec6593d46e4f54" "$CONTROLLER"
check_hash "e9d982b2759c24e5738354f167a9d150ecc98c0dc17dfedbe49257ac10eaf359" "$RESIDUAL"
...
if [[ "$ACTION" == "--check-only" ]]; then
  echo "[OK] racelineV3 H3 candidate hashes match; stableV2 is untouched"
  exit 0
fi
...
GENERATED_DIR="${MPCC_GENERATED_DIR:-$HOME/.cache/f1tenth_residual_mpcc/acados_track_h3_racelinev3_candidate}"
...
if [[ "$PREPARE_ONLY" == "true" ]]; then
  echo "[OK] racelineV3 candidate solvers are ready; no ROS node or command gate was started"
  exit 0
fi
...
if [[ "${MPCC_RECORD_BAG:-false}" == "true" ]]; then
  mkdir -p "$ROOT/bags"
  BAG_PATH="${MPCC_RECORD_BAG_PATH:-$ROOT/bags/track_h3_racelinev3_candidate_${STAMP}.bag}"
...
echo "[CANDIDATE] racelineV3 MPCC closed-loop Markov16 residual, recursive H3 joint fit"
echo "[CANDIDATE] residual trained from 291.58 s of filtered racelinev1/racelineV2 track data"
echo "[CANDIDATE] racelineV3 A3-A6 local reference limit=4.0 m/s; first-run cap remains conservative"
...
LAUNCH_ARGS=(
  ...
  telemetry_path:="$ROOT/log/hardware_track_h3_racelinev3_candidate_${FORMULATION}_${STAMP}.jsonl"
  lap_timer:=true
  lap_log_path:="$ROOT/log/laps_track_h3_racelinev3_candidate_${FORMULATION}_${STAMP}.csv"
)
