...
check_hash() {
  ...
  if [[ "$actual" != "$expected" ]]; then
    echo "[FAIL] formal-track H3 candidate file changed: $path" >&2
...
# the generated solver/runtime pair. This profile is separate from stableV2.
...
if [[ "$ACTION" == "--check-only" ]]; then
  echo "[OK] formal-track H3 candidate hashes match; stableV2 is untouched"
  exit 0
fi
...
GENERATED_DIR="${MPCC_GENERATED_DIR:-$HOME/.cache/f1tenth_residual_mpcc/acados_track_h3_candidate}"
...
if [[ "${MPCC_RECORD_BAG:-false}" == "true" ]]; then
  mkdir -p "$ROOT/bags"
  BAG_PATH="${MPCC_RECORD_BAG_PATH:-$ROOT/bags/track_h3_candidate_${STAMP}.bag}"
...
echo "[CANDIDATE] formal-track MPCC closed-loop Markov16 residual, recursive H3 joint fit"
echo "[CANDIDATE] trained from 291.58 s of filtered racelinev1/racelineV2 track data"
...
LAUNCH_ARGS=(
  ...
  telemetry_path:="$ROOT/log/hardware_track_h3_candidate_${FORMULATION}_${STAMP}.jsonl"
  lap_timer:=true
  lap_log_path:="$ROOT/log/laps_track_h3_candidate_${FORMULATION}_${STAMP}.csv"
)
