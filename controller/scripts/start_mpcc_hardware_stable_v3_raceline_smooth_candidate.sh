...
check_hash() {
  ...
  if [[ "$actual" != "$expected" ]]; then
    echo "[FAIL] V3 raceline_smooth candidate file changed: $path" >&2
...
if [[ "$ACTION" == "--check-only" ]]; then
  echo "[OK] V3 raceline_smooth candidate hashes match; Stable V3 is untouched"
  exit 0
fi
...
GENERATED_DIR="${MPCC_GENERATED_DIR:-$HOME/.cache/f1tenth_residual_mpcc/acados_stable_v3_raceline_smooth_candidate}"
...
if [[ "${MPCC_RECORD_BAG:-false}" == "true" ]]; then
  mkdir -p "$ROOT/bags"
  BAG_PATH="${MPCC_RECORD_BAG_PATH:-$ROOT/bags/stable_v3_raceline_smooth_${STAMP}.bag}"
...
echo "[V3_SMOOTH_CANDIDATE] track=raceline_smooth; residual=H3/0.30"
echo "[V3_SMOOTH_CANDIDATE] contour/heading=45.0/1.0; steering_command_rate=0.60"
echo "[V3_SMOOTH_CANDIDATE] profile_max_decel=3.0 m/s^2; runtime speed cap=${RESIDUAL_MPCC_SPEED_CAP} m/s"
echo "[V3_SMOOTH_CANDIDATE] only boundary widths changed; Stable V3 remains unchanged"
...
LAUNCH_ARGS=(
  ...
  telemetry_path:="$ROOT/log/hardware_stable_v3_raceline_smooth_${FORMULATION}_${STAMP}.jsonl"
  lap_timer:=true
  lap_log_path:="$ROOT/log/laps_stable_v3_raceline_smooth_${FORMULATION}_${STAMP}.csv"
)
