...
check_hash "21c9e3c13727c510aa017e3415bd983d099bc7099bbe0d0a85f81a870b60569b" "$TRACK_CSV"
...
if [[ "$ACTION" == "--check-only" ]]; then
  echo "[OK] stable V4 frozen hashes match"
...
GENERATED_DIR="${MPCC_GENERATED_DIR:-$HOME/.cache/f1tenth_residual_mpcc/acados_stable_v4}"
...
if [[ "${MPCC_RECORD_BAG:-false}" == "true" ]]; then
  mkdir -p "$ROOT/bags"
  BAG_PATH="${MPCC_RECORD_BAG_PATH:-$ROOT/bags/stable_v4_${STAMP}.bag}"
  "$RECORDER" "$BAG_PATH" &
  RECORD_PID=$!
...
LAUNCH_ARGS=(
  ...
  telemetry_path:="$ROOT/log/hardware_stable_v4_${STAMP}.jsonl"
  lap_timer:="${AUTO_RELAUNCH_LAP_TIMER:-true}"
  lap_log_path:="$ROOT/log/laps_stable_v4_${STAMP}.csv"
)
