...
# racelinev3 preserves the A1/A2 speed zones and replaces A3-A6 with the
# corridor-fitted G2 transition.  Its local limit is 4 m/s; runtime dynamics
# remain authoritative below that ceiling.
...
GENERATED_DIR="${MPCC_GENERATED_DIR:-$HOME/.cache/f1tenth_residual_mpcc/acados_stable_v2_racelinev3}"
...
LAUNCH_ARGS=(
  ...
  telemetry_path:="$ROOT/log/hardware_stable_v2_racelinev3_${FORMULATION}_${STAMP}.jsonl"
  lap_timer:=true
  lap_log_path:="$ROOT/log/laps_stable_v2_racelinev3_${FORMULATION}_${STAMP}.csv"
)
