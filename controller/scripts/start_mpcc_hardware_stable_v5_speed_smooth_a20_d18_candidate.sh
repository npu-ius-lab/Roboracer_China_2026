...
TRACK="racelinev3_stable_v5_speed_smooth_a20_d18_candidate"
...
check_hash() {
  ...
  if [[ "$actual" != "$expected" ]]; then
    echo "[FAIL] Stable V5 speed-smooth candidate file changed: $path" >&2
...
check_hash "73e024a832cc9d7b0f7cea4b1e93554b94e2b02d96ef0fd136c66b085fce84b2" "$TRACK_CSV"
...
if [[ "$ACTION" == "--check-only" ]]; then
  echo "[OK] Stable V5 frozen dependencies and isolated speed-envelope track match"
  echo "[OK] Stable V5 itself was not modified"
  echo "[OK] track=$TRACK; envelope accel/decel=2.0/1.8 m/s^2; hard braking remains 4.0 m/s^2"
  exit 0
fi
...
GENERATED_DIR="${MPCC_GENERATED_DIR:-$HOME/.cache/f1tenth_residual_mpcc/acados_stable_v5_speed_smooth_a20_d18_candidate}"
...
if [[ "$PREPARE_ONLY" == "true" ]]; then
  echo "[OK] Stable V5 speed-smooth candidate solvers are ready; hardware was not started"
  exit 0
fi
...
if [[ "${MPCC_RECORD_BAG:-false}" == "true" ]]; then
  ...
  "$RECORDER" "$BAG_PATH" &
  RECORD_PID=$!
  echo "[RECORD] Stable V5 speed-smooth candidate bag -> $BAG_PATH"
  trap 'kill -INT "$RECORD_PID" 2>/dev/null || true' EXIT
fi

echo "[CANDIDATE] Stable V5 geometry/controller + isolated smooth speed envelope"
echo "[CANDIDATE] global=${RESIDUAL_MPCC_SPEED_CAP} m/s; envelope accel/decel=2.0/1.8 m/s^2"
echo "[CANDIDATE] geometry/MPCC/residual unchanged; publisher/fallback braking remains 4.0 m/s^2"
