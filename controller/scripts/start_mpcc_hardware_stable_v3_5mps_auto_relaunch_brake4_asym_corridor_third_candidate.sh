...
BASE_CANDIDATE="stable_v3_5mps_auto_relaunch_brake4_asym_candidate"
CANDIDATE="stable_v3_5mps_auto_relaunch_brake4_asym_corridor_third_candidate"
...
LAUNCH="$ROOT/src/f1tenth_dynamic_mpcc/launch/hardware_mpcc_stable_v3_5mps_auto_relaunch_brake4_asym_corridor_third_candidate.launch"
BASE_CONTROLLER="$ROOT/src/f1tenth_dynamic_mpcc/config/candidates/$BASE_CANDIDATE/controller.yaml"
BASE_LAUNCH="$ROOT/src/f1tenth_dynamic_mpcc/launch/hardware_mpcc_${BASE_CANDIDATE}.launch"
...
# Prove the exact brake4/asymmetric baseline files remain unchanged without
# depending on any unrelated launcher or optional avoidance workspace.
check_hash "962ac26cb6724912e54b90ad3ea4a917c6da72c6c51e001f99fcf6371eb11c08" "$BASE_CONTROLLER"
check_hash "0a56d4a377338a964d5212d2b2e44e7abe5c47c0e9eb34a9ca38580ac8ef2757" "$BASE_LAUNCH"
check_hash "423f2ca62a64a90265234e6433cc2c85f11b5a9aadadefff02415f1b4c3cb98d" "$CONTROLLER"
check_hash "9156b45ab7175e4df651e9a30a12955070344a50ecb50dae9a543215df69a81d" "$LAUNCH"
...
if [[ "$ACTION" == "--check-only" ]]; then
  echo "[OK] brake4/asymmetric automatic-relaunch baseline is unchanged"
  echo "[OK] isolated one-third corridor-margin candidate hashes match"
  exit 0
fi
...
GENERATED_DIR="${MPCC_GENERATED_DIR:-$HOME/.cache/f1tenth_residual_mpcc/acados_stable_v3_5mps_auto_relaunch_brake4_asym_corridor_third}"
...
if [[ "${MPCC_RECORD_BAG:-false}" == "true" ]]; then
  mkdir -p "$ROOT/bags"
  BAG_PATH="${MPCC_RECORD_BAG_PATH:-$ROOT/bags/stable_v3_5mps_auto_relaunch_brake4_asym_corridor_third_${STAMP}.bag}"
  "$RECORDER" "$BAG_PATH" &
  RECORD_PID=$!
...
echo "[CORRIDOR_THIRD] MPCC track-control margin=0.016667 m (base 0.05 m / 3)"
...
LAUNCH_ARGS=(
  ...
  telemetry_path:="$ROOT/log/hardware_stable_v3_5mps_auto_relaunch_brake4_asym_corridor_third_${STAMP}.jsonl"
  lap_timer:="${AUTO_RELAUNCH_LAP_TIMER:-true}"
  lap_log_path:="$ROOT/log/laps_stable_v3_5mps_auto_relaunch_brake4_asym_corridor_third_${STAMP}.csv"
)

if [[ -n "$RECORD_PID" ]]; then
  roslaunch f1tenth_dynamic_mpcc \
    hardware_mpcc_stable_v3_5mps_auto_relaunch_brake4_asym_corridor_third_candidate.launch \
    "${LAUNCH_ARGS[@]}"
else
