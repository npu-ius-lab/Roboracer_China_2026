...
LAUNCH="$ROOT/src/f1tenth_dynamic_mpcc/launch/hardware_mpcc_stable_roboracer_eth_h2h_contact_escape_candidate.launch"
RUNTIME_CONFIG="$ROOT/src/f1tenth_dynamic_mpcc/config/candidates/stable_roboracer_eth_h2h/cpp_runtime.yaml"
RECORDER="$ROOT/scripts/record_roboracer_eth_h2h_contact_escape_bag.sh"
ACTION="${1:---shadow}"
...
if [[ "$ACTION" == "--check-only" ]]; then
  echo "[OK] additive ROBORACER ETH H2H contact-escape candidate"
  echo "[OK] overtaking core unchanged; C++ contact recovery is post-gate only"
  echo "[OK] no node or command publisher started"
  exit 0
fi
...
if [[ "$start_controller" == true ]]; then
  for node in /f1tenth_dynamic_mpcc /f1tenth_dynamic_mpcc_roboracer_chaoche \
      ...
      /roboracer_contact_escape_gate_cpp; do
...
if [[ "$ACTION" == "--hardware" ]]; then
  ...
  if [[ "${MPCC_RECORD_BAG:-false}" == "true" ]]; then
    "$RECORDER" "$ROOT/bags/roboracer_eth_h2h_contact_escape_${STAMP}.bag" &
    pids+=("$!")
  fi
...
echo "[ROBORACER_ETH_H2H_CONTACT_ESCAPE] mode=$ACTION core=C++17 cap=${H2H_SPEED_CAP_MPS:-4.0}m/s"
echo "[ROBORACER_ETH_H2H] upstream perception/localization unchanged"
echo "[ROBORACER_ETH_H2H] planner and overtaking state machine unchanged"
if [[ "$ACTION" == "--hardware" ]]; then
...
roslaunch f1tenth_dynamic_mpcc hardware_mpcc_stable_roboracer_eth_h2h_contact_escape_candidate.launch \
