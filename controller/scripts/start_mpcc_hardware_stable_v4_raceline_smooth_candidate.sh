...
CANDIDATE="stable_v4"
TRACK="racelinev3_5mps_arc_straight_std3_candidate_smooth"
...
LAUNCH="$ROOT/src/f1tenth_dynamic_mpcc/launch/hardware_mpcc_stable_v4.launch"
...
RECORDER="$ROOT/scripts/record_auto_relaunch_bag_raceline_smooth_candidate.sh"
...
check_hash() {
  ...
  if [[ "$actual" != "$expected" ]]; then
    echo "[FAIL] stable V4 frozen file changed: $path" >&2
...
# Stable V4 is a frozen release: validate every behavior-defining file.
check_hash "3d26667c8f185a079d2b0057350a218e76650330852240e2a08d20a5aa4ddce0" "$CONTROLLER"
check_hash "fbe6f459a81deb2814c77c4d9b5b6b50769e36e3fda42e7ede865eb92460a778" "$VEHICLE"
check_hash "14b84138d8dd60de3b7e0e74988ab77089c8afcff71d06bd8615fc6a200dd4f6" "$TRACK_CSV"
check_hash "95a13797eb762132c47928dde83aa454b62d9a344154ddbae4b90b8b612a2d44" "$RESIDUAL"
check_hash "a7192c3f17c3f3383880d1ebb69257149c1f6d0049143b35ac7716ebf73d6e28" "$LAUNCH"
...
if [[ "$ACTION" == "--check-only" ]]; then
  echo "[OK] Stable V4 raceline-smooth candidate hashes match; original Stable V4 is untouched"
  echo "[OK] initial launch and arbitrary-position carry/relaunch are enabled"
  exit 0
fi
...
GENERATED_DIR="${MPCC_GENERATED_DIR:-$HOME/.cache/f1tenth_residual_mpcc/acados_stable_v4_raceline_smooth_candidate}"
...
if [[ "$PREPARE_ONLY" == "true" ]]; then
  echo "[OK] stable V4 solvers are ready; hardware was not started"
  exit 0
fi
...
if [[ "${MPCC_RECORD_BAG:-false}" == "true" ]]; then
  mkdir -p "$ROOT/bags"
  BAG_PATH="${MPCC_RECORD_BAG_PATH:-$ROOT/bags/stable_v4_raceline_smooth_${STAMP}.bag}"
  "$RECORDER" "$BAG_PATH" &
  RECORD_PID=$!
...
echo "[STABLE_V4] frozen from stable_v3_5mps_auto_relaunch_brake4_asym_candidate"
echo "[STABLE_V4] initial launch + arbitrary-position carry/relaunch enabled"
...
LAUNCH_ARGS=(
  ...
  telemetry_path:="$ROOT/log/hardware_stable_v4_raceline_smooth_${STAMP}.jsonl"
  lap_timer:="${AUTO_RELAUNCH_LAP_TIMER:-true}"
  lap_log_path:="$ROOT/log/laps_stable_v4_raceline_smooth_${STAMP}.csv"
)

if [[ -n "$RECORD_PID" ]]; then
  roslaunch f1tenth_dynamic_mpcc \
    hardware_mpcc_stable_v4.launch \
    "${LAUNCH_ARGS[@]}"
else
