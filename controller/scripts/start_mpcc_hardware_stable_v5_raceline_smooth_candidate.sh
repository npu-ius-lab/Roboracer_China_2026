...
CANDIDATE="stable_v5"
TRACK="racelinev3_stable_v5_fast5_std32_smooth"
...
check_hash "0c46200d473fc4ae2113b1157c0f89b47b628be10ddd708bfe31a3ac7a4b902a" "$TRACK_CSV"
...
if [[ "$ACTION" == "--check-only" ]]; then
  echo "[OK] Stable V5 raceline-smooth candidate hashes match; original Stable V5 is untouched"
...
GENERATED_DIR="${MPCC_GENERATED_DIR:-$HOME/.cache/f1tenth_residual_mpcc/acados_stable_v5_raceline_smooth_candidate}"
...
if [[ "${MPCC_RECORD_BAG:-false}" == "true" ]]; then
  mkdir -p "$ROOT/bags"
  BAG_PATH="${MPCC_RECORD_BAG_PATH:-$ROOT/bags/stable_v5_raceline_smooth_${STAMP}.bag}"
  "$RECORDER" "$BAG_PATH" &
  RECORD_PID=$!
...
LAUNCH_ARGS=(
  ...
  telemetry_path:="$ROOT/log/hardware_stable_v5_raceline_smooth_${STAMP}.jsonl"
  lap_timer:="${AUTO_RELAUNCH_LAP_TIMER:-true}"
  lap_log_path:="$ROOT/log/laps_stable_v5_raceline_smooth_${STAMP}.csv"
)
