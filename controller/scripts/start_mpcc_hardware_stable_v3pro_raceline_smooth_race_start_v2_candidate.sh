...
LAUNCH="$ROOT/src/f1tenth_dynamic_mpcc/launch/hardware_mpcc_stable_v3pro_raceline_smooth_race_start_v2_candidate.launch"
...
SUPERVISOR="$ROOT/src/f1tenth_dynamic_mpcc/scripts/automatic_relaunch_supervisor_race_start_v2.py"
...
check_hash "cf6b4d53ead6fb0809c341dd27723bf7204855dabe0adb25602dc2bfc0b5c35f" "$LAUNCH"
...
check_hash "a99e918a2a192572f00cd9d8101894cd8a31f0e79546c44ac8aa5a20bf8c9359" "$SUPERVISOR"
...
if [[ "$ACTION" == "--check-only" ]]; then
  echo "[OK] V3pro raceline_smooth race-start-v2 candidate inputs match"
  echo "[OK] recovery=2.0 m/s^2; race-start=3.2 m/s^2 + dynamic handoff"
  exit 0
fi
...
GENERATED_DIR="${MPCC_GENERATED_DIR:-$HOME/.cache/f1tenth_residual_mpcc/acados_stable_v3pro_raceline_smooth_race_start_v2_candidate}"
...
if [[ "${MPCC_RECORD_BAG:-false}" == "true" ]]; then
  mkdir -p "$ROOT/bags"
  BAG_PATH="${MPCC_RECORD_BAG_PATH:-$ROOT/bags/stable_v3pro_raceline_smooth_race_start_v2_${STAMP}.bag}"
  "$RECORDER" "$BAG_PATH" &
  RECORD_PID=$!
...
echo "[V3PRO_SMOOTH_START_V2] Stable V3 centerline/residual/speed/braking/safety preserved"
echo "[V3PRO_SMOOTH_START_V2] MPCC solve=40 Hz; command publisher/supervisor=50 Hz"
echo "[V3PRO_SMOOTH_START_V2] contour/heading/rate-cost=45.0/1.0/0.60; steering rate=3.4 rad/s"
echo "[V3PRO_SMOOTH_START_V2] profile decel=3.0; publisher accel/decel=1.5/2.0 m/s^2"
echo "[V3PRO_SMOOTH_START_V2] runtime cap=${RESIDUAL_MPCC_SPEED_CAP} m/s; track=raceline_smooth"
...
echo "[V3PRO] grid start PP lateral limit=3.2 m/s^2; speed-relative MPCC handoff"
echo "[V3PRO_SMOOTH_START_V2] Stable V3, V3pro, prior smooth candidate and Stable V5 remain unchanged"
...
LAUNCH_ARGS=(
  ...
  telemetry_path:="$ROOT/log/hardware_stable_v3pro_raceline_smooth_race_start_v2_${STAMP}.jsonl"
  lap_timer:="${AUTO_RELAUNCH_LAP_TIMER:-true}"
  lap_log_path:="$ROOT/log/laps_stable_v3pro_raceline_smooth_race_start_v2_${STAMP}.csv"
)

if [[ -n "$RECORD_PID" ]]; then
  roslaunch f1tenth_dynamic_mpcc hardware_mpcc_stable_v3pro_raceline_smooth_race_start_v2_candidate.launch "${LAUNCH_ARGS[@]}"
else
  exec roslaunch f1tenth_dynamic_mpcc hardware_mpcc_stable_v3pro_raceline_smooth_race_start_v2_candidate.launch "${LAUNCH_ARGS[@]}"
fi
