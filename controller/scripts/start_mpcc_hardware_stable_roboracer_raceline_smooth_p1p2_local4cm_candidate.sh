...
TRACK="raceline_smooth_p1p2_local4cm_candidate"
...
LAUNCH="$ROOT/src/f1tenth_dynamic_mpcc/launch/hardware_mpcc_stable_roboracer_raceline_smooth_p1p2_local4cm_candidate.launch"
RVIZ_CONFIG="$ROOT/src/f1tenth_dynamic_mpcc/rviz/stable_roboracer_raceline_smooth_p1p2_local4cm_candidate.rviz"
...
check_hash "33573533666b619f76f34136370fba14debe4e2d24f9ec103d39892e3eb3b404" "$LAUNCH"
check_hash "2c1cff9928fceaeebb63be09fc914713d84a479a92f248bd741c55e8b88c0ed4" "$RVIZ_CONFIG"
check_hash "0188d1ea37e2f113168fc31c1056912eca8ab37966f35132c8f879910a9d54a6" "$VEHICLE"
check_hash "bf7b59db6221484e99fabbe6c167a6e70807210a2381938e20e331e213373067" "$RACELINE"
...
GENERATED_DIR="${MPCC_GENERATED_DIR:-$HOME/.cache/f1tenth_residual_mpcc/acados_stable_roboracer_raceline_smooth_p1p2_local4cm_candidate}"
...
if [[ "${MPCC_RECORD_BAG:-false}" == "true" ]]; then
  mkdir -p "$ROOT/bags"
  BAG_PATH="${MPCC_RECORD_BAG_PATH:-$ROOT/bags/stable_roboracer_raceline_smooth_p1p2_local4cm_${STAMP}.bag}"
  "$RECORDER" "$BAG_PATH" &
  RECORD_PID=$!
  echo "[RECORD] Stable RoboRacer bag -> $BAG_PATH"
fi

echo "[ROBORACER_P1P2_LOCAL4CM] Frozen from the validated V3pro smooth race-start-v2 candidate"
echo "[ROBORACER_P1P2_LOCAL4CM] MPCC solve=40 Hz; command publisher/supervisor=50 Hz"
echo "[ROBORACER_P1P2_LOCAL4CM] contour/heading/rate-cost=45.0/1.0/0.60; steering rate=3.4 rad/s"
echo "[ROBORACER_P1P2_LOCAL4CM] profile decel=3.0; publisher accel/decel=1.5/2.0 m/s^2"
echo "[ROBORACER_P1P2_LOCAL4CM] runtime cap=${RESIDUAL_MPCC_SPEED_CAP} m/s; track=raceline_smooth_p1p2_local4cm_candidate"
echo "[ROBORACER_P1P2_LOCAL4CM] RViz=/f1tenth_mpcc/stable_roboracer_raceline_smooth_p1p2_local4cm/* (isolated smooth-boundary display)"
echo "[ROBORACER_P1P2_LOCAL4CM] carry detection, stable-ground gate and global-S reprojection enabled"
echo "[ROBORACER_P1P2_LOCAL4CM] arbitrary-position PP recovery=2.0 m/s; smooth PP-to-MPCC handoff"
echo "[ROBORACER_P1P2_LOCAL4CM] grid start PP lateral limit=3.2 m/s^2; speed-relative MPCC handoff"
echo "[ROBORACER_P1P2_LOCAL4CM] grid-start extracted-boundary gate disabled; localization/heading/ground gates retained"
echo "[ROBORACER_P1P2_LOCAL4CM] obstacle perception disabled by default; set ROBORACER_OBSTACLE_PERCEPTION=true to enable"
echo "[ROBORACER_P1P2_LOCAL4CM] control debug: MODE / STATE / OWNER / TAKEOVER transitions are highlighted"
echo "[ROBORACER_P1P2_LOCAL4CM] state topic: /automatic_relaunch_supervisor/state"
echo "[ROBORACER_P1P2_LOCAL4CM] Stable V3, V3pro, RoboRacer and Stable V5 remain unchanged"
...
LAUNCH_ARGS=(
  ...
  telemetry_path:="$ROOT/log/hardware_stable_roboracer_raceline_smooth_p1p2_local4cm_${STAMP}.jsonl"
  lap_timer:="${AUTO_RELAUNCH_LAP_TIMER:-true}"
  lap_log_path:="$ROOT/log/laps_stable_roboracer_raceline_smooth_p1p2_local4cm_${STAMP}.csv"
)

roslaunch f1tenth_dynamic_mpcc hardware_mpcc_stable_roboracer_raceline_smooth_p1p2_local4cm_candidate.launch "${LAUNCH_ARGS[@]}"
