...
class Observer:
    def __init__(self):
        ...
        rospy.Subscriber("/roboracer_react/state", String,
                         self.state_callback, queue_size=50)
        rospy.Subscriber("/roboracer_react/local_speed_cap", Float32,
                         self.cap_callback, queue_size=50)
        rospy.Subscriber("/roboracer_react/mpcc/ackermann_cmd_stamped",
                         AckermannDriveStamped, self.mpcc_callback,
                         queue_size=100)
        rospy.Subscriber("/roboracer_react/local_plan_red", Path,
                         self.path_callback, queue_size=20)
        rospy.Subscriber("/roboracer_react/status", String,
...
if __name__ == "__main__":
    rospy.init_node("roboracer_react_bag_observer")
    Observer()
    rospy.spin()
