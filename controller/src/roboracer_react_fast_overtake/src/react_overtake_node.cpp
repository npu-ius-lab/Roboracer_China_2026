...
double rosAge(const ros::Time& stamp) {
  ...
  // Gazebo can emit one wall-time-stamped sample while /clock is reset during
  // world startup.  A scalar delta avoids ros::Duration range exceptions; the
  // normal age checks then reject that transition sample as stale.
  return now.toSec() - stamp.toSec();
}
...
class ReactOvertakeNode {
 public:
  ReactOvertakeNode() : private_node_("~") {
    ...
    ROS_INFO_STREAM("V3 React Fast planner ready: occupancy_cloud="
                    << cloud_topic_
                    << " odom=" << odom_topic_
 ...
 private:
  ...
  void loadParameters() {
    ...
    // C11 React consumes the post-clustering obstacle cloud. Keep the old ROI
    // key as a backwards-compatible fallback for the earlier React launcher.
    if (!private_node_.getParam("topics/obstacle_cloud", cloud_topic_)) {
    ...
    parameter("topics/local_reference", local_reference_topic_,
              std::string("/roboracer_react_fast/local_reference"));
    parameter("topics/local_plan_red", red_path_topic_,
              std::string("/roboracer_react_fast/local_plan_red"));
    parameter("topics/selected_path", selected_path_topic_,
              std::string("/roboracer_react_fast/selected_path"));
    parameter("topics/candidate_left", left_path_topic_,
              std::string("/roboracer_react_fast/candidate_left"));
    parameter("topics/candidate_right", right_path_topic_,
              std::string("/roboracer_react_fast/candidate_right"));
    parameter("topics/valid_area", valid_area_topic_,
              std::string("/roboracer_react_fast/valid_area"));
    parameter("topics/occupancy_markers", occupancy_marker_topic_,
              std::string("/roboracer_react_fast/occupancy_markers"));
    parameter("topics/speed_cap", speed_cap_topic_,
              std::string("/roboracer_react_fast/local_speed_cap"));
    parameter("topics/state", state_topic_,
              std::string("/roboracer_react_fast/state"));
    parameter("topics/health", health_topic_,
              std::string("/roboracer_react_fast/health"));
    parameter("topics/status", status_topic_,
              std::string("/roboracer_react_fast/status"));
    parameter("topics/diagnostics", diagnostics_topic_,
              std::string("/roboracer_react_fast/diagnostics"));
    ...
    parameter("planner/idle_global_handoff", idle_global_handoff_, true);
    ...
    parameter("control/local_lateral_acceleration_limit_mps2",
              local_lateral_acceleration_limit_mps2_, 3.00);
    ...
    if (allowed_roi_frames_.empty() || update_rate_hz_ <= 0.0 ||
        ...
        cruise_speed_cap_mps_ <= 0.0 || pass_speed_cap_mps_ <= 0.0 ||
        pass_speed_cap_mps_ > cruise_speed_cap_mps_ ||
        return_speed_cap_mps_ <= 0.0 ||
        return_speed_cap_mps_ > cruise_speed_cap_mps_ ||
        local_lateral_acceleration_limit_mps2_ <= 0.0 ||
        blocked_minimum_speed_mps_ < 0.0 ||
        blocked_minimum_speed_mps_ > pass_speed_cap_mps_ ||
        blocked_minimum_speed_mps_ > return_speed_cap_mps_ ||
        blocked_maximum_speed_mps_ < blocked_minimum_speed_mps_ ||
        blocked_maximum_speed_mps_ > cruise_speed_cap_mps_) {
      throw std::runtime_error("invalid V3 React Fast runtime configuration");
    }
  }
  ...
  void odomCallback(const nav_msgs::Odometry::ConstPtr& message) {
    ...
    if (!odom_history_.empty() && state.stamp < odom_history_.back().stamp &&
        odom_history_.back().stamp.toSec() - state.stamp.toSec() > 0.001) {
      last_input_error_ = "out_of_order_odom";
      return;
    }
    odom_history_.push_back(state);
    const double cutoff = state.stamp.toSec() - odom_history_duration_s_;
    while (odom_history_.size() > 2 &&
           odom_history_.front().stamp.toSec() < cutoff) {
  ...
  std::optional<OdomState> odomAt(const ros::Time& stamp) const {
    ...
    if (stamp <= odom_history_.front().stamp) {
      if (odom_history_.front().stamp.toSec() - stamp.toSec() <=
          maximum_odom_extrapolation_s_) return odom_history_.front();
      return std::nullopt;
    }
    if (stamp >= odom_history_.back().stamp) {
      const double dt = stamp.toSec() - odom_history_.back().stamp.toSec();
    ...
    for (std::size_t i = 1; i < odom_history_.size(); ++i) {
      ...
      const double gap = upper.stamp.toSec() - lower.stamp.toSec();
      if (gap <= 0.0 || gap > maximum_odom_bracket_gap_s_) return std::nullopt;
      const double ratio = (stamp.toSec() - lower.stamp.toSec()) / gap;
      OdomState result;
      result.stamp = stamp;
  ...
  void cloudCallback(const sensor_msgs::PointCloud2::ConstPtr& message) {
    ...
    if (!std::isfinite(age) || age > cloud_measurement_max_age_s_ ||
      ...
      last_input_error_ = "stale_or_future_occupancy_cloud";
      return;
    }
    const auto measurement_odom = odomAt(message->header.stamp);
    if (!measurement_odom.has_value()) {
      last_input_error_ = "occupancy_cloud_without_synchronized_odom";
      return;
    }
    try {
      ...
      latest_cloud_point_count_ = points.size();
  ...
  void transition(State next, const std::string& reason) {
    ...
    if (state_ != next) {
      ROS_WARN_STREAM("V3 React Fast " << stateName(state_) << " -> "
  ...
  double localPathSpeedCap(const CandidatePath& path,
                           double configured_cap) const {
    return curvatureLimitedSpeedCap(
        path, configured_cap, local_lateral_acceleration_limit_mps2_,
        blocked_minimum_speed_mps_);
  }
  ...
  void timerCallback(const ros::TimerEvent&) {
    ...
    if (!inputsHealthy()) {
      transition(State::kAbort, "occupancy_cloud_or_odom_stream_stale");
      selected_side_ = "none";
    ...
    if (decision.centerline_blocked) {
      ...
      if (state_ == State::kPass) {
        ...
        // single occupancy frame scores the opposite corridor slightly better.
      ...
      if (selected != nullptr) {
        ...
        output_path = &output;
        speed_cap = localPathSpeedCap(output, pass_speed_cap_mps_);
      ...
      if (!output.feasible) {
        // Once current occupancy is clear, an invalid local return must not
  ...
  void publishOutputs(bool healthy, double speed_cap,
    ...
    status << "{\"state\":\"" << stateName(state_)
           ...
           << ",\"occupancy_cloud_topic\":\"" << cloud_topic_ << "\""
           << ",\"occupancy_cloud_age_s\":"
           << jsonNumber(wallAge(latest_cloud_wall_))
           << ",\"occupancy_cloud_points\":" << latest_cloud_point_count_
  ...
  std::size_t latest_cloud_point_count_{};
...
int main(int argc, char** argv) {
  ros::init(argc, argv, "roboracer_react_fast_overtake");
  try {
    roboracer_react_fast_overtake::ReactOvertakeNode node;
    ros::spin();
  } catch (const std::exception& error) {
    std::cerr << "V3 React Fast planner terminated by exception: "
              << error.what() << std::endl;
    ROS_FATAL("V3 React Fast planner startup failed: %s", error.what());
    return 1;
  }
  return 0;
}
