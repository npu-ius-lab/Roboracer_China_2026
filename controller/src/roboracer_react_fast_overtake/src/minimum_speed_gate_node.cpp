 ...
 public:
  MinimumSpeedGateNode() : private_node_("~") {
    ...
    private_node_.param("raw_command_topic", raw_command_topic_,
                        std::string("/roboracer_react_fast/mpcc/raw_ackermann_cmd_stamped"));
    private_node_.param("output_command_topic", output_command_topic_,
                        std::string("/roboracer_react_fast/mpcc/ackermann_cmd_stamped"));
    ...
    private_node_.param(
        ...
        std::string("/automatic_relaunch_supervisor_roboracer_react_fast/state"));
    private_node_.param("status_topic", status_topic_,
                        std::string("/roboracer_react_fast/min_speed_gate/status"));
    ...
    ROS_INFO_STREAM("V3 React Fast minimum-speed gate: driving-state floor="
...
int main(int argc, char** argv) {
  ros::init(argc, argv, "roboracer_react_fast_minimum_speed_gate");
  try {
    roboracer_react_fast_overtake::MinimumSpeedGateNode node;
    ros::spin();
  } catch (const std::exception& error) {
    ROS_FATAL_STREAM("V3 React Fast minimum-speed gate failed: " << error.what());
    return 1;
  }
  return 0;
}
