...
class MpccNode {
 public:
  MpccNode():private_("~") {
    ...
    tracking_quality_speed_gate_=std::make_unique<mpcc::TrackingQualitySpeedGate>(
      safety["high_speed_tracking_gate"]);
    prediction_soft_margin_=safety["prediction_soft_margin_m"]
      ?safety["prediction_soft_margin_m"].as<double>():0.0;
    if(!std::isfinite(prediction_soft_margin_)||prediction_soft_margin_<0.0)
      throw std::invalid_argument("invalid prediction soft margin");
    ...
    ROS_INFO("StableV5 high-speed tracking gate=%s prediction_soft_margin=%.2fm current_margin_trigger=%.2fm",
             tracking_quality_speed_gate_->enabled()?"true":"false",
             prediction_soft_margin_,pp_current_margin_);
  }
 ...
 private:
  ...
  void controlTick(const ros::TimerEvent&){
    ...
    bool prediction_risk=false,pp_trigger_risk=false,
      ...
      speed_only_risk=false,soft_margin_risk=false,checkpoint_selected=false,
      checkpoint_blocked=false;
    mpcc::TrackingQualitySpeedDecision tracking_gate;
    ...
    try{
      ...
      tracking_gate=tracking_quality_speed_gate_->limit(
        requested_cap,projection.e_contour,
        mpcc::wrapAngle(compensated[2]-projection.psi_ref),
        current_track_margin);
      requested_cap=tracking_gate.speed_cap;
      double target_cap;
      ...
      soft_margin_risk=!result.states.empty()&&
        std::isfinite(mpcc_safety.minimum_margin)&&
        mpcc_safety.minimum_margin<=prediction_soft_margin_;
      prediction_risk=track_rejection||slack_risk||rollout_risk||soft_margin_risk;
