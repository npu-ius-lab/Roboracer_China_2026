...
void AcadosRuntimeSolverJubu::setLocalReference(
...
AcadosRuntimeSolverJubu::~AcadosRuntimeSolverJubu(){
...
void AcadosRuntimeSolverJubu::resetWarmStart(){
  warm_valid_=false;
  for(auto& state:warm_states_){state[4]=0.0;state[5]=0.0;}
}

void AcadosRuntimeSolverJubu::initializeWarmStart(const State& initial,double target_cap,
...
void AcadosRuntimeSolverJubu::advanceWarmStart(double elapsed_s){
...
void AcadosRuntimeSolverJubu::setInputBounds(double speed_cap,double rate_cap){
...
void AcadosRuntimeSolverJubu::updateStageParameters(
...
void AcadosRuntimeSolverJubu::seedSolver(const State& initial,double target_speed_cap,
...
double AcadosRuntimeSolverJubu::feasibleInputSpeedCap(
...
void AcadosRuntimeSolverJubu::readSolution(std::vector<State>& states,
...
TrackSlackSummary AcadosRuntimeSolverJubu::readSlack(double near_horizon_s,
...
CostBreakdown AcadosRuntimeSolverJubu::diagnosticCost(
    const std::vector<State>& states,const std::vector<Control>& controls,
...
SolverResult AcadosRuntimeSolverJubu::solve(const State& initial,double speed_cap,
