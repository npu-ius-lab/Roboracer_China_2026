  ...
  void controlTick(const ros::TimerEvent&){
    ...
    try{
      ...
      if(!checkpoint_selected&&!checkpoint_blocked&&
        ...
        // Normal raceline speed-profile changes use the publisher's normal
        // deceleration. Predictive-risk holds and an out-of-bounds recovery
        // use the emergency rate until their safety condition has cleared.
        const bool emergency_deceleration=prediction_risk||result.speed_retry||
          out_of_bounds||checkpoint_armed||target_cap+1e-6<requested_cap;
