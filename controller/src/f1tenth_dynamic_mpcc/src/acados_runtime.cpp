...
void* AcadosRuntimeSolver::symbol(void* handle, const std::string& name) const {
  dlerror(); void* result=dlsym(handle,name.c_str());
  if(const char* error=dlerror()) throw std::runtime_error("missing symbol "+name+": "+error);
  return result;
}

AcadosRuntimeSolver::AcadosRuntimeSolver(const std::string& formulation,
...
AcadosRuntimeSolver::~AcadosRuntimeSolver(){
...
void AcadosRuntimeSolver::resetWarmStart(){
  warm_valid_=false;
  for(auto& state:warm_states_){state[4]=0.0;state[5]=0.0;}
}

void AcadosRuntimeSolver::initializeWarmStart(const State& initial,double target_cap,
...
void AcadosRuntimeSolver::setInputBounds(double speed_cap,double rate_cap){
...
void AcadosRuntimeSolver::updateStageParameters(
  ...
  for(int k=0;k<=horizon_;++k){
    const auto g=track_.geometry(states[k][8]);
...
void AcadosRuntimeSolver::seedSolver(const State& initial,double target_speed_cap,
  ...
  if(formulation_=="baseline"){
    ...
    for(int k=0;k<=horizon_;++k){
      ...
      speed=clamp(std::min(track_.speedPrior(theta),target_speed_cap),
...
double AcadosRuntimeSolver::feasibleInputSpeedCap(
...
void AcadosRuntimeSolver::readSolution(std::vector<State>& states,
...
TrackSlackSummary AcadosRuntimeSolver::readSlack(double near_horizon_s,
...
CostBreakdown AcadosRuntimeSolver::diagnosticCost(
    const std::vector<State>& states,const std::vector<Control>& controls,
  ...
  for(int k=0;k<horizon_;++k){
    ...
    const auto g=track_.geometry(x[8]);
...
SolverResult AcadosRuntimeSolver::solve(const State& initial,double speed_cap,
  ...
  auto run_rti=[&](int iterations,bool stop_for_far){
    for(int i=0;i<iterations;++i){
      ...
      if(parameter_states.size()==states.size())for(size_t k=0;k<states.size();++k){
        const auto before=track_.geometry(parameter_states[k][8]);
        const auto after=track_.geometry(states[k][8]);
