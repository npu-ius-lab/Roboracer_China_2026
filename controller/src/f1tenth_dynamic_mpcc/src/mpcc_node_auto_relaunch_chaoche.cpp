...
class MpccNode {
 public:
  MpccNode():private_("~") {
    ...
    solver_=std::make_unique<mpcc::AcadosRuntimeSolverChaoche>(formulation,expandHome(generated),*track_,vehicle_,controller_,residual_model_);
    ...
    private_.param("local_reference_max_start_distance_m",
                   local_reference_max_start_distance_,1.25);
    private_.param("local_reference_min_length_m",local_reference_min_length_,1.5);
    private_.param("local_reference_max_end_offset_m",
                   local_reference_max_end_offset_,0.20);
    private_.param("local_reference_default_speed_cap_mps",
                   local_reference_speed_cap_,2.5);
    if(local_reference_timeout_<=0.0||local_reference_max_start_distance_<=0.0||
       local_reference_min_length_<=0.0||local_reference_max_end_offset_<=0.0||
       local_reference_speed_cap_<=0.0)
      throw std::invalid_argument("invalid V5 chaoche local-reference parameters");
    ...
    ROS_INFO("C++ Residual Dynamic MPCC V5 CHAOCHE ready formulation=%s mode=%s odom=%s command=%s enabled=%s speed_cap=%.2f contour=%.2f heading=%.2f steering_rate_cost=%.2f tiered_risk=%s near=%.2fs hold=%.2fs recovery=%.2fm/s2 steer_rate=%.2frad/s oob_recovery=%s local_ref=%s",
 ...
 private:
  ...
  std::shared_ptr<const mpcc::LocalReferenceProfile> buildLocalReference(
    ...
    // The reference manager deliberately republishes the complete locked
    // maneuver so its signature remains stable and the MPCC warm start is not
    // reset on every progress update. Once the car has advanced, the first
    // pose is naturally behind it; requiring that pose to stay close caused a
    // valid reference to be rejected, then timed out the command stream. A
    // locked path remains valid while any finite pose is close to the car.
    double nearest_path_distance_sq=std::numeric_limits<double>::infinity();
    for(const auto& stamped:path.poses){
      const double x=stamped.pose.position.x,y=stamped.pose.position.y;
      if(!std::isfinite(x)||!std::isfinite(y))continue;
      const double dx=x-(*odom)[0],dy=y-(*odom)[1];
      nearest_path_distance_sq=std::min(nearest_path_distance_sq,dx*dx+dy*dy);
    }
    if(nearest_path_distance_sq>
       local_reference_max_start_distance_*local_reference_max_start_distance_)
      throw std::invalid_argument("local path has no point near the vehicle");
    ...
    const auto global_end=track_->geometry(points.back().s);
    const double end_nx=-std::sin(global_end.yaw),end_ny=std::cos(global_end.yaw);
    const double end_ey=(points.back().geometry.x-global_end.x)*end_nx+
                        (points.back().geometry.y-global_end.y)*end_ny;
    if(std::abs(end_ey)>local_reference_max_end_offset_)
      throw std::invalid_argument("local path does not rejoin the global raceline");
    ...
    for(std::size_t i=0;i<points.size();++i){
      const std::size_t before=i==0?0:i-1;
      const std::size_t after=i+1<points.size()?i+1:points.size()-1;
      const double ds=points[after].s-points[before].s;
  ...
  void clearLocalReference(const std::string& reason) {
    ...
    {std::lock_guard<std::mutex> lock(mutex_);
      ...
      local_reference_failsafe_hold_=false;
      state=local_state_;}
  ...
  void localReferenceCallback(const nav_msgs::Path::ConstPtr& message) {
    ...
    try{
      ...
      {std::lock_guard<std::mutex> lock(mutex_);
        ...
        local_reference_stamp_=ros::Time::now().toSec();
        local_reference_failsafe_hold_=false;state=local_state_;}
      publishLocalReferenceStatus(state,true,unchanged?"reference_refreshed":"reference_accepted");
    }catch(const std::exception& error){
      ROS_ERROR_THROTTLE(1,"V5 chaoche local path rejected: %s",error.what());
      std::string state;
  ...
  void localSpeedCapCallback(const std_msgs::Float32::ConstPtr& message) {
    if(!std::isfinite(message->data)||message->data<0.0){
      ROS_WARN_THROTTLE(1,"Ignoring invalid V5 chaoche speed cap %.3f",message->data);
      return;
    }
    std::lock_guard<std::mutex> lock(mutex_);
  ...
  void localStateCallback(const std_msgs::String::ConstPtr& message) {
    static const std::vector<std::string> allowed={
      "GLOBAL","FOLLOW","PREPARE","PASS","RETURN","ABORT"};
    if(std::find(allowed.begin(),allowed.end(),message->data)==allowed.end()){
      ROS_WARN_THROTTLE(1,"Ignoring invalid V5 chaoche state %s",message->data.c_str());
      return;
    }
    std::lock_guard<std::mutex> lock(mutex_);local_state_=message->data;
  }
  ...
  void controlTick(const ros::TimerEvent&){
    ...
    try{
      ...
      // FOLLOW keeps the global raceline but must still obey the longitudinal
      // gap controller. ABORT without a local reference is the fail-closed
      // perception-health state. Previously both states ignored the manager's
      // speed cap, so FOLLOW was only a label and stale perception could fall
      // back to unrestricted global racing.
      const bool behavior_speed_cap_active=local_mpcc_active||
        local_state=="FOLLOW"||
        (local_state=="ABORT"&&!local_mpcc_active);
      if(behavior_speed_cap_active)
      ...
      if(local_mpcc_active){
        ...
        if(result.success&&!result.states.empty()&&mpcc_safety.safe){
          ...
          startup_->noteSolver(true,now);
          ROS_INFO_THROTTLE(1,"V5 chaoche LOCAL MPCC active state=%s cap=%.2f",
                            local_state.c_str(),target_cap);
        }else{
          {std::lock_guard<std::mutex>l(mutex_);oob_recovery_active_=false;}
          command_manager_->noteFailure();startup_->noteSolver(false,now);
          ROS_ERROR_THROTTLE(.5,
            "V5 chaoche local MPCC rejected; no Pure Pursuit fallback state=%s status=%d safe=%s margin=%.3f",
  ...
  std::unique_ptr<mpcc::AcadosRuntimeSolverChaoche>solver_;
  ...
  double collision_stamp_{-1e99},control_rate_{},odom_timeout_{},max_reprop_{},
    ...
    local_reference_max_start_distance_{1.25},local_reference_min_length_{1.5},
    local_reference_max_end_offset_{0.20},
  ...
  std::string formulation_,odom_topic_,command_topic_,collision_topic_,world_frame_,
    ...
    local_reference_status_topic_,local_state_{"GLOBAL"};
  ...
  ros::Subscriber odom_sub_,imu_sub_,wheel_sub_,collision_sub_,local_reference_sub_,
    local_speed_cap_sub_,local_state_sub_;
  ros::ServiceServer enable_service_;
  ros::Timer control_timer_,publisher_timer_;
};

int main(int argc,char**argv){ros::init(argc,argv,"f1tenth_dynamic_mpcc_v5_chaoche");try{MpccNode node;ros::AsyncSpinner spinner(3);spinner.start();ros::waitForShutdown();}catch(const std::exception&e){ROS_FATAL("C++ MPCC V5 chaoche initialization failed: %s",e.what());return 1;}return 0;}
