...
double rosAge(const ros::Time& stamp) {
  ...
  // world startup.  Avoid constructing an out-of-range ros::Duration.
  return now.toSec() - stamp.toSec();
}

// Sensor publishers are paced by /clock in Gazebo.  When the real-time factor
// is below one, a healthy 10 Hz simulated LiDAR can be separated by much more
// than 0.1 seconds of wall time.  Keep wall-time watchdogs on hardware, but use
// message time in simulation so freshness is measured in simulated seconds.
double inputAge(const ros::WallTime& received_wall,
                const ros::Time& message_stamp) {
  return ros::Time::isSimTime() ? rosAge(message_stamp)
                                : wallAge(received_wall);
}
 ...
 private:
  ...
  void odomCallback(const nav_msgs::Odometry::ConstPtr& message) {
    ...
    if (!odom_history_.empty() &&
      ...
      const double reorder =
          odom_history_.back().stamp.toSec() - state.stamp.toSec();
  ...
  bool baseInputsHealthy() const {
    ...
    const double odom_age =
        inputAge(latest_odom_wall_, latest_odom_->stamp);
    const double cloud_input_age =
        inputAge(latest_cloud_wall_, latest_cloud_stamp_);
    if (!std::isfinite(odom_age) || !std::isfinite(cloud_input_age) ||
        odom_age > odom_timeout_s_ || cloud_input_age > cloud_timeout_s_) {
      return false;
    }
    const double measurement_age = rosAge(latest_cloud_stamp_);
    return std::isfinite(measurement_age) &&
           measurement_age <= cloud_measurement_max_age_s_ &&
           measurement_age >= -cloud_future_tolerance_s_;
  }
        ...
        result.push_back(
            {signedTrackDelta(propagated_s, current_s), obstacle.ey, 0.0, 0.0,
             ...
             obstacle.point_count});
               ...
               inputAge(obstacle_memory_.seen_wall,
                        obstacle_memory_.measurement_stamp) <=
    ...
    if (inputs_healthy && state_ != State::kAbort) {
      switch (state_) {
        ...
        case State::kFollow: {
          ...
          if (!decision.relevant &&
              inputAge(last_nonempty_cloud_wall_,
                       obstacle_memory_.measurement_stamp) >
  ...
  void publishOutputs(bool health, double speed_cap,
    ...
    status << "{\"state\":\"" << stateName(state_)
           ...
           << "\",\"input_time_basis\":\""
           << (ros::Time::isSimTime() ? "simulation" : "wall")
           << "\",\"roi_cloud_age_s\":"
           << jsonNumber(inputAge(latest_cloud_wall_, latest_cloud_stamp_))
           << ",\"roi_receive_age_s\":"
           << jsonNumber(wallAge(last_cloud_received_wall_))
           << ",\"odom_age_s\":"
           << jsonNumber(latest_odom_.has_value()
                             ? inputAge(latest_odom_wall_, latest_odom_->stamp)
                             : kInfinity)
