#!/usr/bin/env python3
"""V5 JUBU automatic relaunch supervisor with a planner-health hardware gate."""
...
class AutomaticRelaunchSupervisorJubu:
    ...
    def __init__(self) -> None:
        ...
        self.planner_required = bool(rospy.get_param("~planner_required", True))
        self.planner_health_timeout = float(
            rospy.get_param("~planner_health_timeout_s", 0.50)
        )
        if self.planner_health_timeout <= 0.0:
            raise ValueError("planner_health_timeout_s must be positive")
        ...
        self.planner_healthy = not self.planner_required
        self.planner_health_time = 0.0
        ...
        rospy.Subscriber(
            str(rospy.get_param("~planner_health_topic", "/v5_jubu/health")),
            Bool,
            self.planner_health_callback,
            queue_size=1,
            tcp_nodelay=True,
        )
        ...
        self.publish_zero()
        rospy.logwarn(
            "V5 JUBU supervisor owns REAL commands: initial state=%s; "
            "probe=%.2f m/s; planner health gate=%s",
            self.state,
            self.probe_speed,
            "required" if self.planner_required else "disabled",
        )

    def planner_health_callback(self, message: Bool) -> None:
        with self.lock:
            self.planner_healthy = bool(message.data)
            self.planner_health_time = rospy.get_time()

    def _planner_healthy_locked(self, now: float) -> bool:
        return bool(
            not self.planner_required
            or (
                self.planner_healthy
                and now - self.planner_health_time <= self.planner_health_timeout
            )
        )
    ...
    def _output_command(self) -> AckermannDrive:
        ...
        with self.lock:
            now = rospy.get_time()
            if not self._planner_healthy_locked(now):
                self.state_reason = "planner_health_gate_locked"
                return command
            state = self.state
    ...
    def _status_dictionary(self) -> dict:
        with self.lock:
            ...
            return {
                ...
                "planner_healthy": self._planner_healthy_locked(rospy.get_time()),
                "planner_health_age": (
                    None if self.planner_health_time <= 0.0
                    else max(0.0, rospy.get_time() - self.planner_health_time)
                ),
...
def main() -> None:
    rospy.init_node("automatic_relaunch_supervisor_jubu")
    try:
        AutomaticRelaunchSupervisorJubu()
        rospy.spin()
    except Exception as error:
        rospy.logfatal("V5 JUBU automatic relaunch supervisor failed: %s", error)
        raise
