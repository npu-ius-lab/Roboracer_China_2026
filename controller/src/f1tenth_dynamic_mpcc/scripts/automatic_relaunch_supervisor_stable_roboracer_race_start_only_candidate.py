#!/usr/bin/env python3
"""Race-start-only gentle launch; all recovery behavior stays RoboRacer."""
...
class RaceStartOnlyAutomaticRelaunchSupervisor(AutomaticRelaunchSupervisor):
    """Modify only recognized grid starts and delegate recovery unchanged."""
    ...
    def _clear_safe_geometry(self) -> None:
        self.safe_projection_s = None
        self.safe_corridor_margin = None
        self.safe_contour_error = None
        self.safe_heading_error = None
    ...
    def _lock_for_carry(self, reason: str) -> None:
        # The frozen RoboRacer lock resets its own recovery_progress.  This
        # candidate owns one extra race-start projection cache, so reset it too.
        super()._lock_for_carry(reason)
        self._clear_safe_geometry()
    ...
    def _update_state(self, sample) -> None:
        # Critical scope boundary: arbitrary-position carry recovery is the
        # frozen RoboRacer implementation, with no candidate speed or corridor
        ...
        if reason is not None:
            ...
            rospy.logerr(
                "SAFE RACE START ABORT: %s; margin=%.3f m "
                "heading_error=%.3f rad; hardware output locked",
                reason,
                margin,
                heading,
            )
    ...
    def _status_dictionary(self) -> dict:
        ...
        status.update({
            ...
            "safe_start_scope": "race_start_only",
            "recovery_logic": "frozen_roboracer",
...
def main() -> None:
    ...
    try:
        RaceStartOnlyAutomaticRelaunchSupervisor()
        rospy.spin()
    except Exception as error:
        rospy.logfatal("Race-start-only relaunch supervisor failed: %s", error)
        raise
