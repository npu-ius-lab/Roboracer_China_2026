#!/usr/bin/env python3
"""Frozen Stable RoboRacer supervisor with guarded carry/relaunch hardware mux."""
...
class AutomaticRelaunchSupervisor:
    ...
    OPERATOR_LAUNCH_HEARTBEAT_S = 2.0
    OPERATOR_RUNNING_HEARTBEAT_S = 10.0
    ...
    def __init__(self) -> None:
        ...
        self.recovery_plan_refresh = float(
            rospy.get_param("~recovery_plan_refresh_s", 0.10)
        )
        ...
        self.race_start_ignore_track_safety = bool(
            rospy.get_param("~race_start_ignore_track_safety", False)
        )
        ...
        if (self.probe_lookahead <= 0.0 or
                ...
                self.recovery_plan_refresh <= 0.0 or
        ...
        self.latest_recovery_plan_time = 0.0
        ...
        self.last_hardware_output_speed = 0.0
        self.last_hardware_output_steering = 0.0
        self.last_operator_signature = None
        self.last_operator_log_time = 0.0
    ...
    @staticmethod
    def _operator_mode_label(state: str, launch_mode: str) -> str:
        if state == "WAIT_GROUND":
            return "PENDING(等待识别发车/搬运恢复)"
        if launch_mode == "race_start":
            return "RACE_START(发车模式)"
        return "RECOVERY(任意位置搬运恢复)"

    @staticmethod
    def _operator_control_labels(
        state: str, mpcc_command_fresh: bool
    ) -> tuple[str, str]:
        labels = {
            "WAIT_GROUND": (
                "SAFETY_ZERO(零速锁定)",
                "WAIT_STABLE_GROUND(等待稳定落地)",
            ),
            "CARRY_LOCKED": (
                "SAFETY_ZERO(搬运锁定)",
                "CARRY_TAKEOVER(搬运安全接管)",
            ),
            "WAIT_RELEASE": (
                "PP_ARMED(E-STOP仍保持)",
                "WAIT_ESTOP_RELEASE(等待释放急停)",
            ),
            "ENABLING": (
                "PURE_PURSUIT(PP)",
                "ENABLING_MPCC_PP_HOLD(使能MPCC期间PP保持)",
            ),
            "HANDOFF_PP": (
                "PURE_PURSUIT(PP)",
                "PP_TAKEOVER_WAIT_MPCC_MATCH(PP接管/等待匹配)",
            ),
            "BLEND_TO_MPCC": (
                "PP+MPCC(BLEND)",
                "HANDOFF_PP_TO_MPCC(PP释放给MPCC)",
            ),
        }
        if state == "RUNNING":
            if mpcc_command_fresh:
                return "MPCC", "MPCC_ACTIVE(MPCC已接管)"
            return (
                "SAFETY_ZERO(MPCC指令超时)",
                "MPCC_COMMAND_STALE(无有效控制指令)",
            )
        return labels.get(
            state,
            ("SAFETY_ZERO(未知状态)", "UNKNOWN_STATE(检查监督器)"),
        )

    def _log_operator_status(self, status: dict) -> None:
        """Print state/owner changes loudly and a low-rate launch heartbeat."""
        mode = str(status["operator_mode"])
        state = str(status["state"])
        owner = str(status["controller_owner"])
        takeover = str(status["takeover_stage"])
        signature = (mode, state, owner, takeover)
        mpcc_speed = status["mpcc_command_speed"]
        mpcc_text = "none" if mpcc_speed is None else f"{mpcc_speed:.2f}m/s"
        line = (
            f"MODE={mode} | STATE={state} | OWNER={owner} | "
            f"TAKEOVER={takeover} | REASON={status['reason']} | "
            f"OUT={status['output_command_speed']:.2f}m/s,"
            f"{status['output_command_steering']:+.3f}rad | "
            f"WHEEL={status['wheel_speed']:.2f}m/s | "
            f"PP={status['pp_command_speed']:.2f}m/s | MPCC={mpcc_text} | "
            f"MATCH={status['handoff_matching_samples']}/"
            f"{status['handoff_required_samples']} | "
            f"BLEND={100.0 * status['handoff_blend_progress']:.0f}%"
        )
        now = rospy.get_time()
        transitioned = signature != self.last_operator_signature
        heartbeat_period = (
            self.OPERATOR_RUNNING_HEARTBEAT_S
            if state == "RUNNING" else self.OPERATOR_LAUNCH_HEARTBEAT_S
        )
        heartbeat_due = (
            now < self.last_operator_log_time or
            now - self.last_operator_log_time >= heartbeat_period
        )
        if transitioned:
            rospy.logwarn(
                "\n========== ROBORACER 发车/恢复/接管状态变化 ==========\n"
                "%s\n"
                "=====================================================",
                line,
            )
        elif heartbeat_due:
            rospy.loginfo("[ROBORACER 状态心跳] %s", line)
        else:
            return
        self.last_operator_signature = signature
        self.last_operator_log_time = now
    ...
    def _cached_recovery_plan(
        self, sample: RelaunchMotionSample
    ) -> ForwardRecoveryPlan:
        """Limit expensive global projection while retaining fresh placement."""
        now = rospy.get_time()
        if (self.latest_recovery_plan is None or
                now < self.latest_recovery_plan_time or
                now - self.latest_recovery_plan_time >= self.recovery_plan_refresh):
            self.latest_recovery_plan = self._recovery_plan(sample)
            # Measure from completion so a slow projection cannot immediately
            # trigger another projection and starve the ground-stability window.
            self.latest_recovery_plan_time = rospy.get_time()
        return self.latest_recovery_plan

    def _race_start_pose_candidate(
        self, plan: ForwardRecoveryPlan, sample: RelaunchMotionSample
    ) -> bool:
        """Recognize the grid from pose only, without modeled track margins."""
        placement = plan.placement
        return bool(
            self.race_start_enabled and
            math.hypot(
                sample.x_m - self.race_start_center_x,
                sample.y_m - self.race_start_center_y,
            ) <= self.race_start_radius and
            circular_progress_distance(
                placement.projection.s_wrapped,
                self.race_start_s,
                self.track.length,
            ) <= self.race_start_s_tolerance and
            abs(placement.heading_error_rad)
    ...
    def _lock_for_carry(self, reason: str) -> None:
        ...
        self.ready_placement = None
        self.ready_recovery_plan = None
        self.latest_recovery_plan = None
        self.latest_recovery_plan_time = 0.0
        self.recovery_progress = None
    ...
    def _update_state(self, sample: RelaunchMotionSample) -> None:
        ...
        plan = self._cached_recovery_plan(sample)
        race_start_pose_candidate = self._race_start_pose_candidate(plan, sample)
        if self.race_start_ignore_track_safety and race_start_pose_candidate:
            # The two alternating competition grid slots are physically known.
            # Keep localization, heading and ground-stability gates, but do not
            # let imperfect extracted boundaries suppress the launch command.
            placement = replace(
                plan.placement,
                valid=True,
                reason="race_start_track_safety_bypassed",
            )
            plan = replace(plan, placement=placement, recovery_required=False)
        else:
            placement = plan.placement
        ...
        if self.state in {"WAIT_GROUND", "CARRY_LOCKED"}:
            ...
            if ready and self.controller_disabled:
                ...
                self.launch_mode = (
                    "race_start"
                    if (
                        race_start_pose_candidate
                        if self.race_start_ignore_track_safety
                        else self.race_start_enabled and is_race_start_candidate(
                            plan,
                            sample.x_m,
                            sample.y_m,
                            self.track.length,
    ...
    def _status_dictionary(self) -> dict:
        with self.lock:
            ...
            now = rospy.get_time()
            mpcc_command_fresh = bool(
                self.latest_mpcc_command is not None and
                now - self.latest_mpcc_command_time <= self.command_timeout
            )
            mpcc_command_speed = (
                None if self.latest_mpcc_command is None
                else float(self.latest_mpcc_command.drive.speed)
            )
            mpcc_command_steering = (
                None if self.latest_mpcc_command is None
                else float(self.latest_mpcc_command.drive.steering_angle)
            )
            operator_mode = self._operator_mode_label(
                self.state, self.launch_mode
            )
            controller_owner, takeover_stage = self._operator_control_labels(
                self.state, mpcc_command_fresh
            )
            return {
                "state": self.state,
                ...
                "race_start_track_safety_ignored": (
                    self.race_start_ignore_track_safety
                ),
                ...
                "mpcc_command_fresh": mpcc_command_fresh,
                "mpcc_command_speed": mpcc_command_speed,
                "mpcc_command_steering": mpcc_command_steering,
                "output_command_speed": self.last_hardware_output_speed,
                "output_command_steering": self.last_hardware_output_steering,
                "wheel_speed": self.latest_wheel[1],
            }

    def timer_callback(self, _event) -> None:
        self._process_service_actions()
        command = self._output_command()
        with self.lock:
            self.last_hardware_output_speed = float(command.speed)
            self.last_hardware_output_steering = float(command.steering_angle)
        self.hardware_pub.publish(command)
        status = self._status_dictionary()
        self._log_operator_status(status)
