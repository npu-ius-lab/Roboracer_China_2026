...
class FileZipperApp:
    def __init__(self, root):
        ...
        self.username = os.environ.get("TIANRACER_SFTP_USERNAME", "tianbot")
        # Never ship a vehicle or server password in source control.  Operators
        # may provide it through the environment or enter it in the dialog.
        self.password = os.environ.get("TIANRACER_SFTP_PASSWORD", "")
    ...
    def configure_sftp(self):
        ...
        self.password = simpledialog.askstring("配置SFTP", "请输入密码：", show='*')
    ...
    def upload_via_sftp(self):
        ...
        if not self.password:
            self.password = simpledialog.askstring("配置SFTP", "请输入密码：", show='*')
            if not self.password:
                self.show_error_message("未提供SFTP密码。")
                return
